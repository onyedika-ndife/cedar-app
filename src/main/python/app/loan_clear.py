...
class LOAN_CLEAR(QDialog):
    ...
    def _view(self):
        ...
        elif self.user_data["loan"]["status"].lower() == "pending":
            self.loan_stat.setStyleSheet("""color: purple;""")
        ...
        if i.text() == "&Yes":
            ...
            try:
                ...
                if self.useSavings.isChecked():
                    savings_date = datetime.strptime(
                        self.date.date().toPyDate().strftime("%Y-%m-%d"), "%Y-%m-%d"
                ...
                if loan_status == "cleared":
                    self.db.execute(
                        ...
                        (f"{self.user_data['fullname']} loan schedule",),
                    )
    ...
    def _get_data_db(self, user_id):
        ...
        account["loan"] = {
            "amount": "\u20A6 {:,}".format(int(loans[1])) if loans else "\u20A60.0",
            ...
            "status": loans[6].capitalize() if loans[6] else "\u20A60.0",
        }
